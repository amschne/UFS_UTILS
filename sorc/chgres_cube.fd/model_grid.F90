...
!! @author George Gayno NCEP/EMC   
 ...
 print*,"- tttttt INPUT GRID OBJECT FOR GAUSSIAN DATA."
...
!-----------------------------------------------------------------------
 ...
 input_grid = ESMF_GridCreateMosaic(filename=trim(mosaic_file_input_grid), &
                                  ...
                                  tileFilePath=trim(mosaic_dir_input_grid), &
                                  rc=error)
...
!-----------------------------------------------------------------------
 ...
 do tile = 1, num_tiles_input_grid
   if (localpet == 0) then
     call get_model_latlons(mosaic_file_input_grid, mosaic_dir_input_grid, num_tiles_input_grid, tile, &
...
!-----------------------------------------------------------------------
 ...
 target_grid = ESMF_GridCreateMosaic(filename=trim(mosaic_file_target_grid), &
                                  ...
                                  staggerLocList=(/ESMF_STAGGERLOC_CENTER, ESMF_STAGGERLOC_CORNER, &
                                                   ESMF_STAGGERLOC_EDGE1, ESMF_STAGGERLOC_EDGE2/), &
                                  indexflag=ESMF_INDEX_GLOBAL, &
                                  tileFilePath=trim(mosaic_dir_target_grid), rc=error)
 if(ESMF_logFoundError(rcToCheck=error,msg=ESMF_LOGERR_PASSTHRU,line=__LINE__,file=__FILE__)) &
    call error_handler("IN GridCreateMosaic", error)
...
!-----------------------------------------------------------------------
 ...
 do tile = 1, num_tiles_target_grid
   if (localpet == 0) then
     ...
     call get_model_latlons(mosaic_file_target_grid, mosaic_dir_target_grid, num_tiles_target_grid, tile, &
...
!! @author George Gayno NCEP/EMC   
 subroutine get_model_latlons(mosaic_file, mosaic_dir, num_tiles, tile, &
 ...
 character(len=*), intent(in)      :: mosaic_file, mosaic_dir
 ...
 grid_file = trim(mosaic_dir) // trim(grid_files(tile))
